...
test.describe.serial("アカウント設定の主要フロー（パスワード変更 → メールアドレス変更）", () => {
  ...
  test("メールアドレス変更 → ログアウトされ新メールアドレスで再ログインできる", async ({ page }) => {
    ...
    // メール変更後はサインアウトされ /login へ遷移する。signOut()自体が非同期でCI上の
    // 並列実行時はCPU競合により時間がかかることがあるため、ファイル内の他の遷移待ちと
    // 同じ15秒まで許容する（5秒では今回CIで実際にタイムアウトした）
    await expect(page).toHaveURL(/\/login/, { timeout: 15000 });
    // URL遷移直後はクライアント側のハイドレーションが完了していないことがあり、
    // その状態でフォームをsubmitするとReactのonSubmitではなくネイティブのGET送信になってしまう
    // （並列実行時の負荷で発生。テスト設計書10節の既知のE2E不安定要因と同種）。
    // ハイドレーション完了を待つため、フォームが実際に操作可能になるまで待機する
    await page.getByRole("button", { name: "ログイン", exact: true }).waitFor({ state: "visible" });
    await page.waitForLoadState("networkidle");

    // 新メールアドレス + 新パスワードでログインできることを確認
