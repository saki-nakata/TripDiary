...
test.describe.serial("旅行レポートの主要フロー（カード遷移・エリアバッジ）", () => {
  ...
  test("訪問エリア数カード → エリア別投稿数へスクロールする", async ({ page }) => {
    await page.goto("/mypage?tab=report");

    await page.getByRole("button", { name: /訪問エリア数/ }).click();

    await expect(page.getByText(/エリア別投稿数/)).toBeInViewport();
  });
  ...
  test("エリア別バブル → 検索ページのエリアタブへ絞り込み遷移する", async ({ page }) => {
    await page.goto("/mypage?tab=report");

    await page.getByRole("link", { name: new RegExp(POST.location) }).click();
  ...
  test("年選択 → URLに反映され、ブラウザバックで選択した年が復元される", async ({ page }) => {
    ...
    // 年セレクターはスマホ用・PC用の<select>がCSS表示切替のみで同時にDOM上に存在するため、可視要素に絞る
