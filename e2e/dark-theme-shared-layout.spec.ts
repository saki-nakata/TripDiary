...
test.describe("PR-7a: 共有レイアウトのダークテーマ対応（未ログイン・デスクトップ）", () => {
  for (const colorScheme of ["light", "dark"] as const) {
    test(`未ログイン・デスクトップ幅（${colorScheme}）`, async ({ page }) => {
      ...
      const autoToggle = page.getByRole("radio", { name: "配色: 自動" });
...
test.describe("PR-7a: 共有レイアウトのダークテーマ対応（ログイン後）", () => {
  ...
  for (const colorScheme of ["light", "dark"] as const) {
    test(`ログイン後・デスクトップ幅、ユーザーメニュー内の配色トグル（${colorScheme}）`, async ({ page }) => {
      await page.emulateMedia({ colorScheme });
      await page.setViewportSize({ width: 1280, height: 900 });
      await page.goto("/");
      ...
      // ユーザードロップダウンを開き、メニュー項目と配色トグルのコントラストを確認する
      ...
      const darkToggle = page.getByRole("radio", { name: "配色: ダーク" });
  ...
  test("配色トグルでダークを選択すると即座に反映され、リロード後も維持される", async ({ page }) => {
    await page.goto("/");
    await page.getByRole("button", { name: TEST_USER.nickname }).click();
    await page.getByRole("radio", { name: "配色: ダーク" }).click();
