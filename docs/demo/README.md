...
| `02-post-create.mp4` | 2.2M（2026-08-08再撮影、下記参照） |
| `03-social.mp4` | 991K |
| `04-record-and-reflect.mp4` | 1.9M |
| `05-mobile.mp4` | 336K |

合計約6.9MB。全ファイルとも音声トラックなし（`ffprobe`で確認済み）。GitHub上での再生確認済み（下記「README埋め込み時のトラブルと対応」参照）。

### `02-post-create.mp4`の再撮影（2026-08-08、地図ピン位置の不具合修正後）

初回撮影時、`e2e/demo/02-post-create.demo.spec.ts`の地図クリックが固定ピクセル座標（コンテナ幅766px中の`x:120,y:90`相当）だったため、地図初期表示（東京駅中心・zoom 5）に対して大きくずれ、日本海上（緯度37.04/経度128.18、朝鮮半島東岸沖）にピンが立っていた（エリア選択は「東京都」のまま）。提出前レビューで指摘を受け、以下の対応を行った。

1. 本番DB上の該当投稿（`isProtected`ではない通常の投稿）の`lat`/`lng`を東京駅付近の座標へ直接UPDATE
2. スクリプトの地図クリックをコンテナの厳密な中央（オフセット無し）に変更。ローカルリハーサルで、+15/-10のオフセット付き中央クリックでも zoom 5では隣接県（茨城県）まで飛ぶことを確認したため、オフセットは付けない方式に確定
3. 該当投稿を削除し（コメント・いいね等はcascade、S3画像2枚は未参照のまま残存。既知の残余リスクとして受容）、修正後のスクリプトで`02-post-create.demo.spec.ts`のみ本番に対して再実行
4. 新しい投稿の座標が東京都内（緯度35.6957/経度139.7285）に収まることを本番APIで確認
5. 新しい`.webm`を`ffmpeg`で`.mp4`変換し、GitHub Issue（送信済み・添付確定後は削除可）にドラッグ&ドロップしてuser-attachments URLを取得、`--range 0-0`のレスポンスの`Content-Range`でローカルファイルとの一致（2,294,168バイト）を確認してからREADMEを差し替え
